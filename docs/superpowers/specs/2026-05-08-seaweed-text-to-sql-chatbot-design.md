...
`type` values:
- `scalar` — result is a single cell (one row, one column, e.g. a COUNT or SUM) → rendered as plain text embedded in the answer sentence
- `table` — result has multiple rows, or multiple columns → rendered as `ResultTable` inside the bubble
- `error` — SQL failed or returned no rows → error message + failed SQL shown

The `answer` field is a plain-language sentence summarising the result. It is generated by a second Groq call that receives the SQL result data — this call fires in Step 6, after DuckDB returns rows. For `scalar` types a template ("The answer is X") is used instead to avoid an extra API call.
...
**`ResultTable`**
- Renders the `data` array as a compact HTML table inside the assistant bubble
- Shows max 10 rows by default with a "Show all X rows" expand button if more exist
- Column headers derived from object keys in the first data row
